...
build() {
        scp bmoore@ci:/var/lib/jenkins/jobs/cloud-orchestrator-service/workspace/pkg/version /var/lib/jenkins/jobs/cloud-orchestrator-service/workspace/pkg/version
...
pushBuild() {
        ...
        if [ $? -eq 0 ]
        then
                ssh -i ~/cldo-sioss-rsa-4096 root@"$target" 'apt-get update && apt-get install --force-yes -y cloud-orchestrator'
		scp -i ~/cldo-sioss-rsa-4096 ~/cldo-service-utils/iptables.rules root@"$target":/etc/iptables.rules
		ssh -i ~/cldo-sioss-rsa-4096 root@"$target" 'iptables-restore /etc/iptables.rules'
	fi
}
